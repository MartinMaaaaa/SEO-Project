...
| GSC Workbench | 日期范围、比较覆盖、多指标趋势、Query/Page 下钻、Country/Device/Search Appearance 能力、保存视图、注释和带元数据 CSV 导出 |
| GA4 Behavior | Sessions、Users、Views、Engagement、多指标趋势、Channel、Source/Medium、Landing Page、Device、Country、Key Event 和按需源同步 |
...
- CrUX page/origin/fallback/`No dataset` 状态
- AI 任务创建与历史
- 旧版目录不可访问时的独立运行测试
...
| GSC Workbench | Date/comparison scopes, multi-metric trends, query/page drill-down, Country/Device/Search Appearance capabilities, saved views, annotations, and metadata-aware CSV export |
| GA4 Behavior | Sessions, users, views, engagement, multi-metric trends, channel, source/medium, landing page, device, country, key-event evidence, and explicit source sync |
...
The backend suite covers cached contracts, GSC metric semantics, PageSpeed URL safety/four categories/nulls/failure preservation/latest-only persistence/migration, CrUX page/origin/fallback/no-data states, scoped AI tasks, and operation with all legacy-directory filesystem access blocked.
...
- Decision-oriented contribution and composition charts
- Normalized cross-source page analysis and URL join-quality reporting
- Confirmed GA4 primary SEO conversion events and business priorities
- Evidence-backed GSC/PageSpeed action workflows
